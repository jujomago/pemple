...
class grocery_CRUD_Model  extends CI_Model  {
	...
	$sql1 = "SELECT  column_name as Field, data_type as Type, character_maximum_length, is_nullable as Null, column_default as Default
			FROM information_schema.COLUMNS
			WHERE table_name = '".$table."' order by dtd_identifier";
	$sql2 = "SELECT pg_attribute.attname as pk FROM pg_index, pg_class, pg_attribute WHERE pg_class.oid = '".$table."'::regclass AND indrelid = pg_class.oid AND pg_attribute.attrelid = pg_class.oid AND pg_attribute.attnum = any(pg_index.indkey);";
		...
		foreach($this->db->query($sql1)->result() as  $db_field_type)    	
    	{			
    ...
    function get_field_types_basic_table()
    {
		...
		$sql = "SELECT  column_name as Field, data_type as Type, character_maximum_length, is_nullable as Null, column_default as Default
			FROM information_schema.COLUMNS
			WHERE table_name = '".$this->table_name."' order by dtd_identifier";			
    	...
    	foreach($this->db->query($sql)->result() as $db_field_type)
    	{
    		...
    		//$db_field_types[$db_field_type->field]['db_null'] = $db_field_type->attnotnull == 't' ? true : false;
    		//$db_field_types[$db_field_type->field]['db_extra'] = $db_field_type->extra;
			//$db_field_types[$db_field_type->field]['primary_key'] = $db_field_type->primary_key;			
    	}
		
    	//$results = $this->db->field_data($this->table_name);
		$results = $this->_field_data($this->table_name);
    	foreach($results as $num => $row)
    	{
    		$row = (array)$row;
    		$results[$num] = (object)( array_merge($row, $db_field_types[$row['name']])  );
    	}
    	//log_message('PHP', '-->>>'. print_r($results,true));
    	return $results;
    }
    ...
    function get_field_types($table_name)
    {
    	...
    	$results = $this->_field_data($table_name);
